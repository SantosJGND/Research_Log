...
def count_popKmers(Window, mut_matrix, pop_dict, single= True, frequency_range= [0,1],row=32,col=3,segregating= False,
    ...
    for pop in pop_list:
        ...
        if counted:
            t1= time.time()
            print('##')
            pop_collapsed_mat= Window[pop_dict[pop],:]
            pop_collapsed_mat= np.sum(pop_collapsed_mat,axis=0)
            pop_collapsed_mat[pop_collapsed_mat > 1]
            
            print(len(pop_dict[pop]))
            print(np.sum(pop_collapsed_mat))
            ...
            pop_collapsed_mat, pop_seg_ori= popCount_dissect(klist,Window,mut_matrix,pop_ori, single= single, frequency_range= frequency_range,
                                        scale= scale, prop_gen_used= prop_gen_used, return_private= return_private,PA= PA,minN= minN)

            pop_seg[pop]= pop_seg_ori
            #print(pop_seg_ori.shape)

        pop_summed= np.sum(pop_collapsed_mat,axis= 0)
        t2= time.time()
        ######
        ######        
        array_counts[pop_dict[pop],:]= pop_collapsed_mat * scale * prop_gen_used
    ...
    sum_times= times_filter + times_matrix
    #print('time_elapsed: {}'.format(sum_times))
    #print('mat calc prop:  {}'.format(times_matrix / sum_times))
...
def countkmers_cofactor(pop_gen,mut_matrix, pop_ori, single= True, frequency_range= [0,1],
    scale= 1, prop_gen_used= 1, return_private= False,PA= {}):
    '''
...
def count_popKmers_v2(Window, mut_matrix, pop_dict, single= True, frequency_range= [0,1],row=32,col=3,segregating= False,
    scale= 1, prop_gen_used= 1, return_private= False,PA= {},pop_tag= '_ss',minN= 1e3):
    '''
    Extract population mutation counts from _ind x kmer_ mutation matrix. 
    v2: splits Window sizes to calculate using a maximum number minN of individuals.
    '''
    pop_counts= {}
    num_variants= {}
    pop_seg= {}
