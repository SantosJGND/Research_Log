...
for CHR in chromosomes:
    ...
    for line in Geno:
        Codes = [0,np.nan,2,0,0,0,0,0,0,np.nan]
        d = Miss[Index][0]
        recycle= []
        ...
        for bl in sorted(Basket[CHR].keys()):
            ...
            if d > Out[CHR][bl]:
                ...
                pca = PCA(n_components=5, whiten=False).fit(data)
                ...
                for cl in Biblio[CHR][bl]:
                    ...
                    Clusters_coords.append([CHR,bl,Out[CHR][bl],cl])
...
Group_selection = "cluster"
DOOM = 0
cos_threshold = .6

### Clustering on decomposition
if Group_selection == "cluster":
    
#    bandwidth = estimate_bandwidth(COMPS81, quantile=0.1)
#    if bandwidth==0:
#        bandwidth = 0.1
#    
...
#    db = DBSCAN(min_samples=35).fit(COMPS81)
#    labels1= db.labels_
#    label_select = {y:[x for x in range(len(labels1)) if labels1[x] == y] for y in sorted(list(set(labels1))) if y != -1}
...
#    ### K-means
    from sklearn.cluster import KMeans
    kmeans = KMeans(n_clusters=10, random_state=0).fit(COMPS81)
    labels1 = kmeans.labels_
    label_select = {y:[x for x in range(len(labels1)) if labels1[x] == y] for y in sorted(list(set(labels1)))}


if Group_selection == "manual":
    ###" for haplotype cluster only. could be by distance.
    label_select = {y:[x for x in range(len(Trend)) if Trend[x] == y] for y in sorted(list(set(Trend)))}
...
if args.reduc:
    ...
    params = {'bandwidth': np.linspace(np.min(COMPS81), np.max(COMPS81),30)}
    grid = GridSearchCV(KernelDensity(algorithm = "ball_tree",breadth_first = False), params,verbose=0)
...
if args.plot == True:
    ...
    Blancs= {aim:{Chr:{bl:[0]*len(Focus) for bl in Ref_profiles[Chr].keys()} for Chr in chromosomes} for aim in list(set(labels1))}
    
    target_block= {Chr:{bl:[0]*len(Focus) for bl in Ref_profiles[Chr].keys()} for Chr in chromosomes}
    ...
    for unit in range(len(labels1)):
        ...
        for native in Ind_labels[site[0]][site[1]][site[3]]:
            if Fam[Whose[native]] in Focus:
                Blancs[labels1[unit]][site[0]][site[1]][Focus.index(Fam[Whose[native]])] = 1
